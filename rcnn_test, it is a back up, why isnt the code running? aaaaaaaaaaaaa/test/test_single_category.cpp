...
int main(int argc, char** argv) {
	if (argc != 2) {
		cerr<<"Wrong number of input, specify the category you want to test"<<endl;
		exit(0);
	}
	int cat = atoi(argv[1]);

	string ROOT_DIR = "/home/user/rccn_for_cloth/data/clothesDataset/test";

	/* test for all the category in the dataset */
	for (int iCategory = cat; iCategory <= cat; iCategory ++) {
	...
	cout<<"% -------------------------------------------------------- %" <<endl;
	cout<<"The image results is stored in"<< ROOT_DIR + "/" + to_string(cat) + "/result/"<<endl;
	cout<<"% -------------------------------------------------------- %" <<endl;
	return 0;
}
