...
void rgbConvert(_matrix<unsigned char>& src,_matrix<float>& dst,int flag,int useSingle)
{
	int n = src.size[0] * src.size[1];
	int d = src.size[2];
	rgbConvert(src.value, dst.value, n, d, flag, 1.0f/255);
}
...
void calcO(_matrix<float>& E,_matrix<float>& O)
{
	...
	for (int i = 0; i < O.num;i++){
		float t = atan(Oyy.value[i] * sgn(-Oxy.value[i]) / (Oxx.value[i] + 1e-5));
		O.value[i] = fmod( t,PI);
	}
