...
void EdgeBoxGenerator::clusterEdges( arrayf &E, arrayf &O, arrayf &V )
{
	...
	for( c=1; c<w-1; c++ ) for( r=1; r<h-1; r++ ) {
		...
		while( sumv < _edgeMergeThr ) {
			...
			float minv=1000; j=0;
			for( i=0; i<(int)vs.size(); i++ ) if( vs[i]<minv ) {
	...
	for( c=1; c<w-1; c++ ) for( r=1; r<h-1; r++ )
		if( (j=_segIds.val(c,r))>0 ) _segMag[j]+=E.val(c,r);
	for( i=0; i<(int)_segMag.size(); i++ ) if( _segMag[i]>0 ) map[i]=_segCnt++;
	for( c=1; c<w-1; c++ ) for( r=1; r<h-1; r++ )
	...
	for( c=rad; c<w-rad; c++ ) for( r=rad; r<h-rad; r++ ) {
		...
		for( cd=-rad; cd<=rad; cd++ ) for( rd=-rad; rd<=rad; rd++ ) {
			...
			bool found = false; for(i=0;i<(int)_segAffIdx[s0].size();i++)
...
void edgeBoxes(float*EE, float* OO, int h, int w, float _alpha, float _beta, float _minScore, float _maxBoxes, float _edgeMinMag, float _edgeMergeThr, float _clusterMinMag,
	float _maxAspectRatio, float _minBoxArea, float _gamma, float _kappa, _bbs& bbs)
{
	arrayf E; E._x = EE;
	arrayf O; O._x = OO;
