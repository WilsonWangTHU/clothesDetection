...
def mytest(net, imageName):
    ...
    obj_proposals = sio.loadmat('/home/wangtw/fast-rcnn/data/demo/testbox.mat')['boxes']
    ...
    for cls in np.arange(len(CLASSES)):
        ...
        for i in indexs:
            ...
            ax.text(bbox[0], bbox[1] - 2, '{:d} {:.3f}'.format(cls, score),bbox=dict(facecolor='blue', alpha=0.5), fontsize=14, color='white') 

        ax.set_title(('{} detections with '
                      'p({} | box) >= {:.1f}').format(cls, cls, CONF_THRESH), fontsize=14)
...
def parse_args():
    ...
    parser.add_argument('--netmodel', dest='caffemodel', help='testImage',
                        default='\home\wtw\123.jpg', type=str)
